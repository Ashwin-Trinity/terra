...
def read_from_source():
    ...
    query="""select workflow_id,source_details,source_load_type,source_load_details,last_run_details,computation_env from workflow_details
                where workflow_id='{}'""".format(workflow_id)
    cursor.execute(query)
    conn.commit()
    record = cursor.fetchone()
    logger.info("Version : "+Terra_version)
    ...
    try:
        ...
        df = (spark.read
            ...
            .option("encoding", source_details['file_encoding'])
        ...
        # EMPTY FILE 
        if number_of_rows==0:
            if 'allow_empty_file' in source_details:
                if source_details['allow_empty_file']:
                    logger.info("Extracting Empty file from source")
                else:
        ...
        try:
            if "\n" in e:
                error_details = {"error_log": ''.join(f'{e}'.split("\n")[:-1])}
                run_details_dict.update(error_details)
                run_details = json.dumps(run_details_dict)
            else:
                error_details = {"error_log": str(e)}
                run_details_dict.update(error_details)
                run_details = json.dumps(run_details_dict)
        except:
            error_details = {"error_log": 'Failed to capturre the error log in metadata'}
            run_details_dict.update(error_details)
            run_details = json.dumps(run_details_dict)
...
def write_to_target():   
    ...
    # fetch workflow details
    query = """select workflow_id,target_details,epoch_id,etl_details,target_load_type,target_load_details,source_load_type,source_load_details from workflow_details
                where workflow_id='{}'""".format(workflow_id)
    cursor.execute(query)
    conn.commit()
    record = cursor.fetchone()
    ...
    eil_detials = json.loads(record['etl_details'])
    ...
    # Epoch Id is generated using UTC timezone
    ...
    try: 
        ...
        etl_start=datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S.000+00:00')
        etl_update=etl_start
        ...
        if dq_checks_flag=='Y':
            ...
            fail_job_count = fail_job.count()
            if fail_job_count == 0:
                
                source_df = source_df.withColumn("etl_Create_date", lit(etl_start).cast("timestamp"))
                source_df=source_df.withColumn("etl_Update_date",lit(etl_start).cast("timestamp"))
                ...
                if record['epoch_id'] == None:
                    ...
                    epoch_id = {"epoch_id":current_epoch_id}
                ...
                quarantine_data.withColumn("etl_Create_date",lit(etl_start).cast("timestamp")).withColumn("etl_Update_date",lit(etl_update).cast("timestamp")).withColumn('epoch_id', lit(current_epoch_id)).write.mode("overwrite").format("parquet").partitionBy("epoch_id").saveAsTable(f"{current_database}.{current_table}_quarantine_data")
                ...
                raise ValueError("Data not loaded to target due to action fail job in DQ step")
        ...
        # Target run status
        run_details_dict.update({**dq_run_details,**target_column_details, **target_row_details, **epoch_id})
        run_details = json.dumps(run_details_dict)
...
def email_step(time_zone):  
    ...
    try:
        ...
        def get_source_target_details():
            source_target_details_query = """select workflow_id_id, run_id, step_name, step_completion_status, run_details from workflow_execution_details where workflow_id_id = {} and run_id = {} and run_details is not null""".format(workflow_id, current_run_id)
            cursor.execute(source_target_details_query)
            wed=cursor.fetchall()
            if wed[-1]['step_name']=="Write_to_target" and wed[-1]['step_completion_status']=="success":
                source_target_details = wed[-1]['run_details']
                source_target_details = json.loads(source_target_details)
                if 'source_row_count' in source_target_details and 'target_row_count' in source_target_details:
                    source_row_count = source_target_details['source_row_count']
                    target_row_count = source_target_details['target_row_count']
            ...
            elif wed[0]['step_name']=="Read_from_source" and wed[0]['step_completion_status']=="success":
                source_target_details = wed[0]['run_details']
                source_target_details = json.loads(source_target_details) 
                if 'source_row_count' in source_target_details: 
                    source_row_count = source_target_details['source_row_count']
            ...
            return (str(source_row_count), str(target_row_count), empty)
            
        source_row_count, target_row_count, empty = get_source_target_details()
        
        
        mail = mailer_module.Mailer(to=cs.to_mail_list)
        mail.build_subject_status(job_name = workflow_name, status = w_status, time_zone = time_zone, empty=empty)
        ...
        mail.build_status_mail_body(workflow_name, schd_time, current_time, w_status, source_row_count, target_row_count)
        mail.send_email()
        time.sleep(1)
